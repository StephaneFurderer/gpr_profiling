...
import six
import pandas as pd
import gpr_profiling.formatters as formatters
import gpr_profiling.templates as templates
import gpr_profiling.plot as plot
...
def to_html(sample, stats_object, stats_df, excluded_metric_names):
    ...
    stats_df : pandas dataframe
        Containing three columns with statistics generated externally - name, desc, val
    excluded_metric_names: list
        List of metrics to be excluded from warning generations
    ...
    if not isinstance(stats_object, dict):
        raise TypeError("stats_object must be of type dict. Did you generate this using the gpr_profiling.describe() function?")

    # Removed need for correlation plots
    ...
    def fmt(value, name):
        if isinstance(value, list):
            return str(value)
        if isinstance(value, abc.ItemsView):
    ...
    # Generate GPR Warnings & detect suitable metrics
    messages_html_gpr = u''
    gpr_metric_values = ""
    gpr_metric_descriptions = ""

    for i in range(len(stats_df)):

        name = stats_df.iloc[i][0]
        desc = stats_df.iloc[i][1]
        metric = stats_df.iloc[i][2]

        gpr_metric_descriptions += templates.gpr_desc_row.format(name = name, desc = desc)

        if name in excluded_metric_names:

            gpr_metric_values += templates.gpr_metric_row.format(name=name,formatted_values=fmt(metric, name))
            ...
            if isinstance(metric, bool):
                ...
                if not metric:
                    msg = templates.messages['check-warning'].format(name = name, val = metric)
                    gpr_metric_values += templates.gpr_metric_row.format(name=name, formatted_values=fmt(metric, name))
                else:
                    msg = templates.messages['check-passed'].format(name=name, val = metric)
                ...
                if metric:
                    msg = templates.messages['list-warning'].format(name = name, val = len(metric))
                    gpr_metric_values += templates.gpr_metric_row.format(name=name, formatted_values=fmt(metric, name))
                else:
                    msg = templates.messages['list-passed'].format(name=name, val = len(metric))

            elif isinstance(metric, abc.ItemsView):

                if metric:
                    msg = templates.messages['list-warning'].format(name = name, val = len(metric))
                    gpr_metric_values += templates.gpr_metric_row.format(name=name, formatted_values=fmt(metric, name))
                else:
                    msg = templates.messages['list-passed'].format(name=name)
                ...
                msg = templates.messages['check-error'].format(name=name)

            messages_html_gpr += templates.message_row.format(message=msg)

    overview_html = templates.template('overview').render(values=formatted_values, gpr_values = gpr_metric_values,
                                                          row_classes=row_classes, messages=messages_html,
                                                          messages_gpr=messages_html_gpr, value_descs =  gpr_metric_descriptions)
